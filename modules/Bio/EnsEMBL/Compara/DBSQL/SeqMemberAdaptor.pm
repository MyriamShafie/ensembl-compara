...
sub _fetch_all_canonical_for_blast_by_genome_db_id {
    my ($self, $genome_db_id, $start_member_id, $end_member_id) = @_;

    my $join = [[['gene_member', 'mg'], 'mg.canonical_member_id = m.seq_member_id'], [['seq_member_projection', 'left_projection'], 'left_projection.target_seq_member_id IS NULL'] ];
    my $constraint = 'mg.genome_db_id = ?';
